...
void heapEngine(T arr[], int size, int root) {
    ...
    // Если левое значение больше переопределяем корень.
    if (left < size && arr[left] > arr[largest])
        largest = left;

    // Если правое значение больше переопределяем корень.
    if (right < size && arr[right] > arr[largest])
        largest = right;

    // Если корень не изменился.
    if (largest != root) {
        std::swap(arr[root], arr[largest]);

        // Рекурсивный вызов heapEngine the affected sub-tree
